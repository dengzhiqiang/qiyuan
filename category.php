...
setcookie("category", "1", time() + 2);   //设定cookie存活时间1s
...
if (!$smarty->is_cached('category.dwt', $cache_id)) {
    ...
    /**
     * 数据库静态化备份2017-11-06，不在进行属性刷选
     * 因为当前网站并没有用到这个功能，并且产品较少
     */
    /* 属性筛选 */
    $ext = ''; //商品查询条件扩展
    //	if ($cat['filter_attr'] > 0) 这个是原来的
...
//    $smarty->assign('txt_links',       $links['txt']);
    ...
    /**
     * 数据库静态化备份2017-11-06，品牌和促销活动不要
     *
     */
//    $brand_list = array_merge($arr, get_brands($cat_id, 'category')); // 获得某个分类下品牌

    $smarty->assign('data_dir', DATA_DIR);

//    $smarty->assign('brand_list',      $brand_list);
//    $smarty->assign('promotion_info', get_promotion_info()); // 所有的促销活动信息
    ...
    /**
     * 数据库静态化备份2017-11-06，问卷调查，最好的商品，促销商品，热商品不要
     *
     */
...
{
    global $memcache; // 全局的$memcache


    $cache_var = "category_info_" . $cat_id;//定义mem 变量

//    $memcache->del($cache_var);

    $row = $memcache->get($cache_var);
    ...
    if ($row === false) {
        ...
        // 高速缓存保留3个小时高速缓存保留3个小时
        $memcache->set($cache_var, $row, 3 * 60 * 60);
    }
