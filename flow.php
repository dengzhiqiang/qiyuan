...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        if((isset($_FILES['best_time']['error']) && $_FILES['best_time']['error'] == 0) || (!isset($_FILES['best_time']['error']) && isset($_FILES['best_time']['tmp_name']) && $_FILES['best_time']['tmp_name'] != 'none')){
            $img_name = upload_file($_FILES['best_time'], 'besttime');
        }else{
            $img_name = '';
        }
        /*
