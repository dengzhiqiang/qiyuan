...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        // 清除缓存，并没有清除session
//        var_dump($_SESSION['user_rank']);
//        var_dump($row['shop_price_formated']);
//        die();

        /*输出当前活动允许报名的等级*/
        if($row['allow_rank_id']) {
            $rank_where = ' where rank_id = ' . $row['allow_rank_id'];
            $rank_name_sql = "select rank_name from " .  $GLOBALS['ecs']->table('user_rank') . $rank_where  ;
            $rank_name = $GLOBALS['db']->getRow($rank_name_sql);
            $row['rank_name'] = $rank_name['rank_name'];
//        var_dump($rank_name);
//        die;
        }
