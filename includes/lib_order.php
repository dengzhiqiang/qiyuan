    ...
    switch ($storage)
    {
        ...
        case 1 :
            $sql = "SELECT goods_id, SUM(goods_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['ecs']->table('order_goods') .
                    " WHERE order_id = '$order_id' AND (is_real in (0,1)) GROUP BY goods_id, product_id";
//            is_real = 0 表示虚拟商品，这里 (is_real in (0,1)) 就是为了让虚拟商品也减少1
        break;

        case 'xuni' :
             /*虚拟活动*/
            $sql = "SELECT goods_id, SUM(goods_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['ecs']->table('order_goods') .
                " WHERE order_id = '$order_id' AND is_real = 0 GROUP BY goods_id, product_id";
            break;
    }
...
//    file_put_contents(ROOT_PATH . '1xuni.txt',$str);
