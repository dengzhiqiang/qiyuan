...
<style>
  ...
  .order-list-orderOut input[type="text"]{
    padding: 2px 5px;
  }
  .order-list-orderOut button{
    height:22px;
    line-height:22px;
    text-align: center;
  ...
  .order-list-orderOut button:hover{
    background: #00964b;
  }

  #order-list-orderOut-name{
    margin-right: 30px;
  }
...
<div class="order-list-orderOut">

  <input type="text" id="orderListStartName" placeholder="请输入报名名称">
  <button id="order-list-orderOut-name" onclick="order_to_excel_name()">按照报名名称导出</button>

  <input id="orderListStartTime" type="text" placeholder="请输入报名时间">
  <input id="orderListEndTime" type="text" placeholder="请输入报名时间">
  <button id="order-list-orderOut-time" onclick="order_to_excel_time()">按照报报名时间导出</button>
</div>
...
<script>
    //执行一个laydate实例
    laydate.render({
        elem: '#orderListStartTime' //指定元素
    });
    laydate.render({
        elem: '#orderListEndTime' //指定元素
    });
</script>
<!--// 邓 导出订单-->
...
?>
  <tr>
    <td valign="top" nowrap="nowrap"><input type="checkbox" name="checkboxes" value="<?php echo $this->_var['order']['order_sn']; ?>" /><a href="order.php?act=info&order_id=<?php echo $this->_var['order']['order_id']; ?>" id="order_<?php echo $this->_var['okey']; ?>"><?php echo $this->_var['order']['order_sn']; ?><?php echo $this->_var['order']['log_id']; ?><?php if ($this->_var['order']['extension_code'] == "group_buy"): ?><br /><div align="center"><?php echo $this->_var['lang']['group_buy']; ?></div><?php elseif ($this->_var['order']['extension_code'] == "exchange_goods"): ?><br /><div align="center"><?php echo $this->_var['lang']['exchange_goods']; ?></div><?php endif; ?></a></td>
...
<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
     ...
     *  按照时间导出订单excel
     */
    function order_to_excel_time() {
    var orderListStartTime = document.getElementById('orderListStartTime').value; // 开始时间
    var orderListEndTime = document.getElementById('orderListEndTime').value; // 结束时间

    if(orderListStartTime.replace(/^\s+|\s+$/g,'').length > 0 && orderListEndTime.replace(/^\s+|\s+$/g,'').length > 0) {

        window.location.href='../plugins/order_to_excel/index.php?orderListStartTime='+orderListStartTime+'&orderListEndTime='+orderListEndTime;
    }else{
        alert('请选择时间！');
    }
}
     ...
     * 按照名称导出订单excel
     */
function order_to_excel_name(){

    var orderListStartName = document.getElementById('orderListStartName').value; // 订单名称
        if(orderListStartName.replace(/^\s+|\s+$/g,'').length > 0) {

            window.location.href='../plugins/order_to_excel/orderListStartName.php?orderListStartName='+orderListStartName;
        }else{
            alert('请输入报名名称！');
        }
}
