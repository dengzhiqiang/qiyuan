...
<link href="themes/xiaomi/cart.css" rel="stylesheet" type="text/css" />

<?php echo $this->smarty_insert_scripts(array('files'=>'common.js,shopping_flow.js')); ?>
</head>
<body>
<?php echo $this->fetch('library/page_header_flow.lbi'); ?> <?php echo $this->smarty_insert_scripts(array('files'=>'xiaomi_flow.js')); ?>
...
<div class="page-main" id="cart-box">
	<div class="container">
        ...
        <div class="cart-empty">
        	<h2>您添加的活动为空!</h2>
            <a href="./" class="btn btn-primary">参加活动</a>
        </div>
        <?php else: ?>
    	...
    	<div class="cart-goods-list">
            <div class="list-head clearfix">
                ...
                <div class="col col-name">活动名称</div>
                <div class="col col-price">费用</div>
                <div class="col col-num" style="display:none;">数量</div>
                <div class="col col-total" style="width:270px;padding-right: 0;">小计</div>
                <div class="col col-action" style="width:200px;padding-right: 0;text-align: right;">操作</div>
            </div>
            <div class="list-body">
...
?>
                <div class="item-box">
                  ...
                  <div class="col col-img"> <img src="themes/xiaomi/images/czlb.png"></div>
...
?>
                  ...
                  <?php else: ?>
                  <div class="col col-img"> <img src="themes/xiaomi/images/yhcx.png"></div>
                  ...
                  <div class="col col-num">
                  	<div class="change-goods-num clearfix" style="display:none;">
                        <!--<a href="javascript:void(0)" class="minus" title="减少1个数量" onclick="flowClickCartNum(<?php echo $this->_var['goods']['rec_id']; ?>,-1);" ><i class="iconfont"></i></a>-->
                        <input type="text" id="goods_number_<?php echo $this->_var['goods']['rec_id']; ?>" value="1" readonly>
                        <!--<a href="javascript:void(0)" class="add" title="增加1个数量" onclick="flowClickCartNum(<?php echo $this->_var['goods']['rec_id']; ?>,+1);"><i class="iconfont"></i></a>-->
                    </div>
                  </div>
                  <?php else: ?>
                 <div class="col col-num" style="text-indent:35px; font-size:14px;"> <?php echo $this->_var['goods']['goods_number']; ?> </div>
                  ...
                  <div class="col col-action">
                  	<a class="del" href="javascript:if (confirm('<?php echo $this->_var['lang']['drop_goods_confirm']; ?>')) location.href='flow.php?step=drop_goods&amp;id=<?php echo $this->_var['goods']['rec_id']; ?>';"><i class="iconfont"></i></a>
                  </div>
                  		</div>
                  	</div>
                </div>
            ...
            <p class="clear-cart"> <a id="del-all" href="flow.php?step=clear">清空活动</a> </p>
            
            <div class="cart-bar clearfix">
              <div class="section-left">
                <a class="back-shopping btn btn-gray" href="./">添加活动</a>
              </div>
              <span class="total-price"><span class="total-num"></span>&nbsp;&nbsp;&nbsp;合计：<b id="totalSkuPrice"><?php echo $this->_var['total']['goods_price']; ?></b></span>
              <a href="flow.php?step=checkout" class="btn btn-pay btn-primary">报名</a>
            </div>
    	</div>
                ...
                <li class="span4">
                    <dl>
                      ...
                      <dd class="xm-recommend-tips">
                          <a href="javascript:fittings_to_flow(<?php echo $this->_var['fittings']['goods_id']; ?>,<?php echo $this->_var['fittings']['parent_id']; ?>)" class="btn btn-small btn-line-primary" id="add-cart">放入报名表</a>
                      </dd>
                    </dl>
                </li>
                <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
...
padding: 10px 20px 12px; height:auto; cursor:pointer; border:none;" value="加入报名表" /></td>
            </tr>
            <?php endif; ?>
    </table>
...
?>
          ...
          var mobile_invaild = '请输入正确的身份证号码！！！';
...
?>
    <div class="container">
    <form action="flow.php" method="post" name="theForm" id="theForm" enctype="multipart/form-data" onsubmit="return checkConsignee(this)">
      ...
      <div class="flowBox_in">
        <form action="flow.php" method="post" name="theForm" id="theForm" onsubmit="return checkOrderForm(this)">
          ...
          <li class="section-options clearfix">
            <h3 class="section-header"><span>报名人信息：</span></h3>
            <div class="section-body">
            	<div class="checkout-item active"><?php echo htmlspecialchars($this->_var['consignee']['consignee']); ?></div>
                ...
                <a href="flow.php?step=consignee" class="modify" style=" height: 38px;
    line-height: 38px;
    padding: 0 30px;
    display: inline-block;background:#ff6700;color:#fff !important;">修改</a>
            </div>
          </li>
...
?>
                	<li>
                    	...
                    	<input type="radio" name="payment" class="radio" id="payment_<?php echo $this->_foreach['foo']['iteration']; ?>" value="<?php echo $this->_var['payment']['pay_id']; ?>" <?php if ($this->_var['order']['pay_id'] == $this->_var['payment']['pay_id']): ?>checked<?php endif; ?> isCod="<?php echo $this->_var['payment']['is_cod']; ?>" onclick="selectPayment(this)" <?php if ($this->_var['cod_disabled'] && $this->_var['payment']['is_cod'] == "1"): ?>disabled="true"<?php endif; ?>/>
                        <div class="text">
...
?>
                <li style="display:none;" <?php if (($this->_foreach['shipping']['iteration'] - 1) == 0): ?>  class="active" <?php endif; ?>>
                    ...
                    <input name="shipping" type="radio" id="ECS_NEEDINSURE_<?php echo $this->_foreach['shipping']['iteration']; ?>" value="<?php echo $this->_var['shipping']['shipping_id']; ?>" <?php if ($this->_var['order']['shipping_id'] == $this->_var['shipping']['shipping_id']): ?>checked="true"<?php endif; ?> supportCod="<?php echo $this->_var['shipping']['support_cod']; ?>" insure="<?php echo $this->_var['shipping']['insure']; ?>" class="radio" onclick="selectShipping(this)" />
                    <div class="text">
                ...
                <div style="margin-top:20px;" style="display: none;">
          ...
          <?php endif; ?>
              ...
              <li class="section-options clearfix section-goods">
                  <div class="section-header clearfix">
                      ...
                      <?php if ($this->_var['allow_edit_cart']): ?><a style="display:inline-block;height: 30px;line-height:30px;padding: 0 30px;background:#ff6700;color:#fff !important;" href="flow.php" class="modify">修改活动订单<i class="iconfont"></i></a><?php endif; ?>
                  </div>
...
?>
                              ...
                              <?php endif; ?>
                              <?php endif; ?>
                              <?php if ($this->_var['goods']['is_shipping']): ?><span style="color:#FF0000;display:none;"><?php echo $this->_var['lang']['free_goods']; ?></span><?php endif; ?></td>
                          <td bgcolor="#ffffff" align="center"><?php echo $this->_var['goods']['formated_goods_price']; ?>&nbsp;x&nbsp;<?php echo $this->_var['goods']['goods_number']; ?></td>
                          <td bgcolor="#ffffff" align="center"><span style="color:#ff6700;"><?php echo $this->_var['goods']['formated_subtotal']; ?></span></td>
                          <td bgcolor="#ffffff" align="center"><a href="flow.php" style="color:#ff6700;">修改</a></td>
                      </tr>
                      <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
...
?>
          ...
          <li class="section-options clearfix" style="border-bottom:none;">
              ...
              <input type="image" src="themes/xiaomi/images/bnt_subOrder.gif" />
  ...
  <div class="container">
  	<div class="section section-order">
    	<div class="order-info clearfix">
        	<div class="fl">
                <h2 class="title">感谢您的参与，您的订单已经提交成功，请记住您的订单号:<b><?php echo $this->_var['order']['order_sn']; ?></b></h2>
                <h2>请于“<span id="dengCountReduceTimeID">29分59秒</span>”之内缴纳报名费用，过时订单失效，您需要重新进行报名!</h2>
...
?>
        ...
        if (!Utils.usernameIsCardID(frm.elements['username'].value)) {
            alert('请输入合法的身份证号码！');
            return false;
        }
...
<script>
    /**
     * 倒计时
     * @type {{init: {_orderTimer: null, data: Date}, leftTimer: dengCountReduceTime.leftTimer, checkTime: dengCountReduceTime.checkTime, go: dengCountReduceTime.go}}
     */
    var dengCountReduceTime = {
        init: {
            _orderTimer: null,
            data: new Date(Date.parse(new Date()) + (3600 * 0.5 * 1000))
        },
        leftTimer: function (enddate) {
            var _this = this;

            if(!document.getElementById("dengCountReduceTimeID")){
                window.clearInterval(_this._orderTimer);
                return;
            }
            ...
            if (days >= 0 || hours >= 0 || minutes >= 0 || seconds >= 0)
                document.getElementById("dengCountReduceTimeID").innerHTML = minutes + "分" + seconds + "秒";

            if (days <= 0 && hours <= 0 && minutes <= 0 && seconds <= 0) {
                // 取消订单
                if(document.getElementsByClassName('pay_action')){
                    document.getElementsByClassName('pay_action')[0].remove();
                    $('#dengCountReduceTimeID').parent().html('订单已失效！');
        ...
        go: function go() {
            ...
            _this._orderTimer = setInterval(function () {
